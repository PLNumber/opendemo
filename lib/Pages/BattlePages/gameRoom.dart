...
class _GameRoomPageState extends State<GameRoomPage> {
  ...
  void initState() {
    ...
    gameFunctions.addPlayerToRoom(widget.roomId, widget.playerId).then((_) {
      ...
      // 방 데이터 상태 체크 리스너 추가
      _setupRoomDataListener();
      _startHeartbeat(); // Heartbeat 시작
    }).catchError((error) {
  ...
  @override
  void dispose() {
    _opponentCheckTimer?.cancel(); // 상대방 체크 Timer 정리
    _heartbeatTimer?.cancel(); // Heartbeat Timer 정리
    super.dispose();
  }
...
/*방 확인 용*/
  // 방 데이터 상태 체크 리스너
  void _setupRoomDataListener() {
    gameFunctions.roomsRef.child(widget.roomId).onValue.listen((event) {
      if (event.snapshot.exists) {
        final roomData = event.snapshot.value as Map;
        final players = roomData['players'] ?? {};
        if (!players.containsKey(widget.playerId)) {
          // 방에 플레이어가 없다면 강제로 방을 나갑니다.
          _forceLeaveRoom();
        }
      }
    });
      ...
      if (playerId != widget.playerId) {
        if (mounted) {
          setState(() {
            ...
            gameFunctions.startOpponentCheck(widget.roomId); // 상대방 체크 시작
          });
        }
      }
    });
...
// 상대방에게 플레이어 나갔다는 알림 전송
  void _notifyOpponentPlayerLeft(String roomId, String playerId) {
    if (gameFunctions.isGameFinished) return; // 게임이 종료된 경우 메시지 전송 방지
...
// 상대방의 메시지를 수신하여 UI 업데이트
  void _setupPlayerLeftListener() {
    gameFunctions.roomsRef.child(widget.roomId).child('messages').onChildAdded.listen((event) {
      final messageData = event.snapshot.value as Map;
      if (messageData['type'] == 'player_left') {
        final leftPlayerId = messageData['playerId'];
        if (mounted) {
          setState(() {
            messages.add("$leftPlayerId가 방을 나갔습니다.");
